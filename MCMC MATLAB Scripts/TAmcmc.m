function [models,logP]=TAmcmc(minit,logPfuns,mccount,varargin)
%% Cascaded affine invariant ensemble MCMC sampler. "The MCMC hammer"
% TAMCMC is a modification of GWMCMC that adds an option for tempering
% starting from a unitless temperature of 4000 and cooling to a unitless
% temperature of 1 over ~150 steps.  Other than that, the usage is the same
% as the original GWMCMC script documented below.
%
%   -Matthew Ashner, 2018
%
...
%   'Temper': Temper the chain (default=false)
%
% OUTPUTS:
...
if isoctave
    ...
    p=p.addParamValue('Temper',false,@islogical);
    p=p.parse(varargin{:});
else
    ...
    p.addParameter('Temper',false,@islogical);
    p.parse(varargin{:});
end
p=p.Results;
...
%Set up tempering with a power law temperature decay.  Temperature profile
%can be adjusted by changing the T0 and slope parameters.  The default
%parameters are well optimized for 200 steps per walker after thinning.
%They may need adjusting for different step numbers.
if p.Temper
    T0=4000; %Initial temperature
    slope=0.97;  %Controlls rate of temperature decay
    beta=1./(T0*0.97.^((1:Nkeep)-1)+1);
else
    beta=ones(Nkeep,1);  %Keeps temperature of 1 if no tempering.
end
            ...
            parfor wix=1:Nwalkers
                ...
                if lr(1)<(numel(proposedm(:,wix))-1)*log(zz(wix))
                    for fix=1:NPfun
                        ...
                        if lr(fix+1)>beta(row)*(proposedlogP(fix)-cp(fix)) || ~isreal(proposedlogP(fix)) || isnan( proposedlogP(fix) )
            ...
            for wix=1:Nwalkers
                ...
                if logrand(1,wix)<(numel(proposedm(:,wix))-1)*log(zz(wix))
                    for fix=1:NPfun
                        ...
                        if logrand(fix+1,wix)>beta(row)*(proposedlogP(fix)-curlogP(fix,wix)) || ~isreal(proposedlogP(fix)) || isnan(proposedlogP(fix))
